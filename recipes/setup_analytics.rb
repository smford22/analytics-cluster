...
#
# 1) Provisioning the entire `analytics-cluster::setup` or
# 2) Just the Chef Server `analytics-cluster::setup_chef_server`
#
...
machine analytics_server_hostname do
  ...
  add_machine_options bootstrap_options: { instance_type: node['analytics-cluster']['analytics']['flavor']  } if node['analytics-cluster']['analytics']['flavor']
  files lazy {{
...
machine analytics_server_hostname do
  chef_server lazy { chef_server_config }
  recipe "analytics-cluster::analytics"
  files(
